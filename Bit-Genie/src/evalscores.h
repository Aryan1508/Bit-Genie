...
namespace PawnEval
{
    constexpr int stacked = S( -8, -29);

    constexpr int isolated = S(-21,  -9);

    constexpr int passed_connected = S( 13,  28);

    constexpr int value = S(117, 172);

    constexpr int psqt[64]
    {
        S(  0,   0), S(  0,   0), S(  0,   0), S(  0,   0), S(  0,   0), S(  0,   0), S(  0,   0), S(  0,   0), 
        S(  6,  22), S( -9,  15), S(-44, -12), S(-66, -28), S(-40, -12), S(-38,  -8), S(-35,  11), S(-58,  38), 
        S(-19,  68), S( -6,  48), S( -1,  11), S(-26, -45), S( -3, -44), S( 58, -16), S( 47,  13), S( 11,  28), 
        S(-30,  54), S(-18,  19), S( -8,   3), S( -6, -21), S( 11, -28), S(  7, -14), S(  2,   1), S(-11,  18), 
        S(-37,  12), S(-27,  -7), S(-13, -23), S(  0, -29), S(  3, -29), S( -2, -23), S(-15, -16), S(-21, -17), 
        S(-35,   0), S(-25,  -7), S(-14, -10), S(-13, -31), S(  4, -23), S( -2,  -9), S(  5, -22), S(-17, -15), 
        S(-31,   0), S(-21, -12), S(-17, -10), S(-14, -17), S(  0,  -5), S( 10,  -2), S( 13, -11), S(-23, -10), 
        S(  0,   0), S(  0,   0), S(  0,   0), S(  0,   0), S(  0,   0), S(  0,   0), S(  0,   0), S(  0,   0), 
    };
...
{
    constexpr int value = S(414, 395);

    constexpr int psqt[64]
    {
        S(-94, -82), S(-37, -36), S( -5,  -7), S(-22, -19), S( -6,  -2), S(-46, -45), S(-46, -47), S(-101, -101), 
        S(-26, -10), S(-16,   7), S(  9,  -1), S(  5,  10), S(  3,  -4), S( -3,  -5), S( -2,  -4), S(-36, -25), 
        S(-26, -20), S(  2,   0), S(  7,   1), S( 18,  17), S( 16,  21), S( 23,  23), S( 13,  11), S( -3,   0), 
        S( -4,   1), S(  5,   4), S( 11,  11), S( 35,  32), S( 26,  22), S( 25,  22), S( 11,   9), S( 13,  11), 
        S( -7,  -5), S( -6,  -6), S(  7,   6), S( 13,  19), S( 16,  13), S( 14,  13), S( 15,  14), S(  6,   3), 
        S(-22, -17), S(-17, -14), S( -9, -12), S(  0,   1), S(  8,   6), S( -4,  -6), S(  4,   0), S(-20, -21), 
        S(-12, -12), S(-18,  -9), S(-21, -14), S(  1, -13), S( -1,  -8), S( -6, -21), S(-13, -10), S( -2, -15), 
        S(-34, -34), S(-15, -14), S(-31, -23), S(-19, -15), S( -8,  -8), S(-16, -20), S(-12, -30), S(-18, -11), 
    };

    constexpr int mobility[9]
    {
        S(-125, -115), S(-49, -63), S(-13,  15), S(  0,  46), S( 20,  44), S( 26,  46), S( 34,  40), S( 44,  38), 
        S( 51,  29), 
    };
}
...
namespace BishopEval
{
    ...
    {
        S(-80, -70), S(-76, -67), S(-34,  13), S(-12,  52), S(  3,  29), S( 11,  51), S( 28,  79), S( 39,  84), 
        S( 43,  75), S( 46,  75), S( 51,  68), S( 51,  57), S( 50,  35), S( 59,  39), 
    };
}

namespace RookEval
{
    constexpr int value = S(607, 778);
    ...
    {
        S(-57, -69), S(-28, -58), S(-17,  46), S( -4,  66), S(  1,  84), S( 11,  71), S( 16,  61), S( 22, 101), 
        S( 25,  45), S( 32,  54), S( 36,  62), S( 40,  57), S( 40,  49), S( 45,  57), S( 39,  21), 
    };

    constexpr int open_file = S( 18,  21);

    constexpr int semi_open_file = S( 18,  21);
}

namespace QueenEval
{
    constexpr int value = S(1319, 1455);
    ...
    {
        S(-62, -48), S(-70, -36), S(-66, -49), S(-45, -50), S(-27, -46), S(  4, -45), S(  0, -37), S( -4, -23), 
        S(  2,  -7), S(  5,  16), S( 10,  28), S( 15,  35), S( 21,  41), S( 26,  47), S( 31,  53), S( 34,  62), 
        S( 36,  69), S( 41,  72), S( 41,  73), S( 44,  73), S( 48,  75), S( 53,  63), S( 57,  69), S( 66,  66), 
        S( 62,  85), S( 83,  84), S(104, 111), S(108, 131), 
    };
}
...
namespace KingEval
{
    ...
    {
        S( 13,  13), S(  0,   0), S(-12,  -7), S(  0,   8), S(  2,   4), S( -4,  -9), S(  1,  20), S(-17,  -7), 
        S(-15,  -6), S(-40, -31), S(-58, -53), S(-64, -58), S(-63, -50), S(-64, -54), S(-93, -92), S(-95, -75), 
        S(-114, -81), S(-104, -81), S(-112, -87), S(-140, -107), S(-183, -125), S(-158, -142), S(-163, -139), S(-242, -223), 
        S(-150, -142), S(-205, -126), S(-263, -223), S(-131, -120), S(-234, -151), S(-141, -127), S(-79, -16), S(-175, -122), 
