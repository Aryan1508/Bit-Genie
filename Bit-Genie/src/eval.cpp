...
static constexpr int mobility_score(Position const& position, Square sq)
{
    static_assert((type == Queen || type == Rook || type == Bishop || type == Knight) && "Invalid piece type in mobility_score!");
	
    uint64_t attacks = Attacks::generate(type, sq, position.total_occupancy());
    return mobility_scores[type - 1][popcount64(attacks)];
}

static constexpr int passed_pawn_scores[total_ranks] = {
	S( 0, 0 ),   S( -16, -16 ), S( -24, -24 ), S( 2, 2 ),
    S( 55, 54 ), S( 162, 162 ), S( 228, 228 ), S( 0, 0),
};
...
static int evaluate_knight(Position const& position, Square sq, Color us)
{
	...
	score += mobility_score<Knight>(position, sq);
...
static int evaluate_rook(Position const& position, Square sq, Color us)
{
	...
	score += mobility_score<Rook>(position, sq);
...
static int evaluate_queen(Position const& position, Square sq, Color us)
{
	...
	score += mobility_score<Queen>(position, sq);
...
static int evaluate_bishop(Position const& position, Square sq, Color us)
{
	...
	score += mobility_score<Bishop>(position, sq);
...
static int evaluate_piece(Position const& position, Callable F, uint64_t pieces, Color us)
{
	...
	while (pieces)
		score += F(position, pop_lsb(pieces), us);
...
static int evaluate_piece(Position const& position, Callable F)
{
	...
	score += evaluate_piece(position, F, white, White);
	score -= evaluate_piece(position, F, black, Black);
...
static int material_balance(Position const& position)
{
	...
	for (int i = 0; i < 5; i++)
	{
		...
		uint64_t white = position.pieces.get_piece_bb(PieceType(i), White);
		uint64_t black = position.pieces.get_piece_bb(PieceType(i), Black);
