...
void TTable::add(
    Position const &position, Move move, int16_t score, std::uint8_t depth,
    TEFlag flag, int16_t seval) {
    std::uint64_t hash  = position.get_key();
    std::uint64_t index = hash % entries.size();

    if (flag != TEFlag::exact && hash == entries[index].hash &&
