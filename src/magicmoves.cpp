...
std::uint64_t initmagicmoves_occ(
    const int *squares, const int numSquares, const std::uint64_t linocc) {
    int i;
    std::uint64_t ret = 0;
...
void init_magic_moves(void) {
    ...
    for (i = 0; i < 64; i++) {
        ...
        for (temp = 0; temp < (((std::uint64_t)(1)) << numsquares); temp++) {
            std::uint64_t tempocc     = initmagicmoves_occ(squares, numsquares, temp);
