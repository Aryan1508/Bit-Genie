...
namespace Generator
{
    ...
    {
        search = std::make_unique<Search::Info>();
        search->position = std::make_unique<Position>();
    }
    ...
    std::pair<Move, int> Game::search_position()
    {
        search->limits.max_depth = 9;
        return Search::bestmove(*search, false);
    }
        ...
        while(true)
        {
            ...
            if (is_mated())
            {
                if (search->position->king_in_check())
                    result = search->position->get_side() == Color::White ? BLACK_WIN_STRING : WHITE_WIN_STRING;
                else 
                    result = DRAW_STRING;
                break;
            }

            auto [move, score] = search_position();
            int white_relative_score = search->position->get_side() == Color::White ? score : -score;
